...
const cursorPosition = reactive({ value: { x: 0, y: 0 } });
...
const navigate = () => {
  setTimeout(async () => {
    await navigateTo(
      `/work/${
        toRaw(data.value).home?.selectedProjects[isNext.value]?.projectSlug
          .current
      }`,
      { redirectCode: 301 }
    );
...
let updateCursorPosition = function (cp, scroll) {
  // console.log(cp)
  cp.style.transform =
    "translate(" +
        ...
        <template v-if="data.home?.selectedProjects[isNext]">
          <div class="work-footer">
            ...
            <template v-if="!isMobile">
              <NuxtLink
                :to="`/work/${data.home?.selectedProjects[isNext].projectSlug.current}`"
              ...
              >
                ...
                <span class="anima-fade">
                  <h2 class="work-footer-scroll-next">
                    {{ data.home?.selectedProjects[isNext]?.projectTitle }}
                  </h2>
                </span>
                <span class="anima-fade">
                ...
                <span>
                  <div
                    ref="scrollImage"
                    v-if="data.home?.selectedProjects[isNext]"
                    class="work-footer-scroll-image"
                  >
                    <!-- <div class='work-footer-scroll-image-overlay'></div> -->
                    <template
                      v-if="
                        !data.home.selectedProjects[isNext].projectDetailImage
                      "
                    >
                      <template
                        v-if="
                          data.home?.selectedProjects[isNext]?.projectCaseImage
                            ?.projectCaseSelection === 'image'
                        "
                      >
                        <SanityImage
                          class="a"
                          :asset-id="
                            data.home?.selectedProjects[isNext]
